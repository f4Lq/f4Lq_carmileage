...
Citizen.CreateThread(function()
	while true do
		if IsPedInAnyVehicle(PlayerPedId(), false) then
				...
				local driver = GetPedInVehicleSeat(veh, -1)
				if driver == PlayerPedId() and GetVehicleClass(veh) ~= 13 and GetVehicleClass(veh) ~= 14 and GetVehicleClass(veh) ~= 15 and GetVehicleClass(veh) ~= 16 and GetVehicleClass(veh) ~= 17 and GetVehicleClass(veh) ~= 21 then
					local message = "Przebieg pojazdu: " .. round(showKM, 2) .. "km"
					TriggerEvent('f4LqNotify:Alert', 'Przebieg', message, 5000, 'info') -- 5s = 5000 milisekund
			end
		else
			Citizen.Wait(3000) -- 4s = 4000 milisekund
		end
		Citizen.Wait(1800000) -- 30min = 1800000 milisekund
	end
end) 
...
-- Zostanie to wykorzystane w przyszłości, aby dodać uszkodzenia samochodów o więcej km i sprawić, że są wolniejsze
...
function round(num, numDecimalPlaces)
  ...
  return math.floor(num * mult + 0.1) / mult
end